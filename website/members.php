...
<html>
	...
	<body>
		...
		<?php
			...
			if (isset($_POST['makeadmin']) == "makeadmin") {
				if (isset($_POST['fuid'])) {
					$fuid = $_POST['fuid'];  
					$query = "update users2groups set admin=1 where uid = $fuid and gid = $groupid";
					$result = mysqli_query($database, $query);
					#echo "Error: " . $query . "<br>" . mysqli_error($database);
				}
			}	
			
			if (isset($_POST['notadmin']) == "notadmin") {
				if (isset($_POST['fuid'])) {
					$fuid = $_POST['fuid'];  
					$query = "update users2groups set admin=0 where uid = $fuid and gid = $groupid";
					$result = mysqli_query($database, $query);
					#echo "Error: " . $query . "<br>" . mysqli_error($database);
				}
			}	
			...
			if (isset($_POST['remove']) == "remove") {
				if (isset($_POST['fuid'])) {
					$fuid = $_POST['fuid'];  
					$query = "delete from users2groups where uid = $fuid and gid = $groupid";
					$result = mysqli_query($database, $query);
					#echo "Error: " . $query . "<br>" . mysqli_error($database);
				}
			}	
			...
			$query = "select groupname from groups where gid = $groupid";
			$result = mysqli_query($database, $query); 
			$thisrow=mysqli_fetch_assoc($result);

			$groupname 	= $thisrow['groupname'];
			
			$query = "select admin from users2groups where gid = $groupid and uid=$uid";
			$result = mysqli_query($database, $query); 
			$thisrow=mysqli_fetch_assoc($result);
		...
		<main>
			<div class="c-hero c--blue">
				<h1><?php echo $groupname; ?> Members</h1>
			</div>
			<div class="t-main">
				<h1>Search</h1>
					...
					<?php
						...
						if (isset($_POST['search']) == "search") {
							if ((isset($_POST['trainers']))) {
								...
								$query =     "SELECT users.uid, firstname, lastname, profession, location, profileimagetype, coach, trainer, lookingforclients, u2u.uid1 as u1, u2u.uid2 as u2, u4u.uid1 as u3, u4u.uid2 as u4, u2u.connection , u4u.connection, admin FROM users2groups g, users left outer join users2users u2u on u2u.uid2=users.uid and u2u.uid1 =$uid and connection=1 , profiles left outer join users2users u4u on u4u.uid2=profiles.uid and u4u.uid1 =$uid and connection=2 WHERE users.uid = profiles.uid and g.gid=$groupid and g.uid = users.uid and lookingforclients=1";
								if (strlen($qs) > 0){
									$query .= " and (".$qs.")"; 
								}
							} else {
								...
								$query =     "SELECT users.uid, firstname, lastname, profession, location, profileimagetype, coach, trainer, lookingforclients, u2u.uid1 as u1, u2u.uid2 as u2, u4u.uid1 as u3, u4u.uid2 as u4, u2u.connection , u4u.connection, admin FROM users2groups g, users left outer join users2users u2u on u2u.uid2=users.uid and u2u.uid1 =$uid and connection=1 , profiles left outer join users2users u4u on u4u.uid2=profiles.uid and u4u.uid1 =$uid and connection=2 WHERE users.uid = profiles.uid and g.gid=$groupid and g.uid = users.uid";
								if (strlen($qs) > 0){
									$query .= " and (".$qs.")"; 
								}
							}
...
<!--																	<button type="submit" id="unfollow" name="unfollow" value="unfollow" class="c-button c--primary">Unfollow</button>-->
																...
																<?php
																if ($admin > 0) { ?>
																	<button type="submit" id="remove" name="remove" value="remove" class="submitLink u-text-bold u-margin-right-sm">Remove</button>
																	<!--<button class="cc-friend__request c-button c--primary u-margin-right-sm">Train with me</button>-->
																	<?php } ?>
																	&nbsp; &nbsp; &nbsp;
																	<?php if ($isadmin == 0) { ?>
																		<button type="submit" id="makeadmin" name="makeadmin" value="makeadmin" class="submitLink u-text-bold u-margin-right-sm">Make Admin</button>
																	<?php } else { ?>
																		<button type="submit" id="notadmin" name="notadmin" value="notadmin" class="submitLink u-text-bold u-margin-right-sm">Not Admin</button>
																	<?php } ?>	
																	&nbsp; &nbsp; &nbsp;
														</form>
													</div>
